...
export default function AdminPage() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      // Monitoramento usando a Normalização para o Alarme
  ...
  const updateStatus = async (id: string, newStatus: string, pedido?: any) => {
    try {
      ...
      // Lógica de avisos automáticos via WhatsApp baseada no novo status
  ...
  // --- LÓGICA DE FILTRAGEM POR ABA COM NORMALIZAÇÃO ---
  ...
  if (!currentUser || !admins.includes(currentUser.email!)) {
    return (
      <div style={{ height: "100vh", display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column", gap: "20px", fontFamily: "'Inter', sans-serif", backgroundColor: "#fcfcfc" }}>
        ...
        <h1 style={{ margin: 0, color: "#111", fontWeight: "900" }}>Acesso Restrito</h1>
        <p style={{ color: "#666", textAlign: "center", maxWidth: "300px" }}>Esta área é exclusiva para administradores da Família Lanches.</p>
        <button onClick={() => window.location.href = "/"} style={{ padding: "14px 28px", borderRadius: "14px", border: "none", background: "#111", color: "#fff", fontWeight: "bold", cursor: "pointer", boxShadow: "0 4px 12px rgba(0,0,0,0.1)" }}>Voltar para o Início</button>
      </div>
    );
  }
  return (
      ...
      <header style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "40px", borderBottom: "1px solid #eee", paddingBottom: "30px" }}>
        <div>
          <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
            ...
            {alarmeAtivo && (
              <span style={{ background: "#ff4444", color: "#fff", padding: "4px 12px", borderRadius: "50px", fontSize: "12px", fontWeight: "bold", animation: "pulse 1s infinite" }}>
                NOVO PEDIDO! 🔔
              </span>
            )}
          </div>
          ...
          <div style={{ display: "flex", gap: "15px", marginTop: "20px", alignItems: "center", flexWrap: "wrap" }}>
            ...
            <div style={{ display: "flex", background: "#f0f0f0", padding: "5px", borderRadius: "15px" }}>
              <button onClick={() => setTab("cozinha")} style={{ padding: "10px 20px", border: "none", borderRadius: "12px", cursor: "pointer", background: tab === "cozinha" ? "#fff" : "transparent", fontWeight: "800", color: tab === "cozinha" ? "#111" : "#888", boxShadow: tab === "cozinha" ? "0 4px 12px rgba(0,0,0,0.05)" : "none" }}>🔥 Cozinha ({pedidos.filter(p => ["Pendente", "Em Produção"].includes(normalizarStatus(p.status))).length})</button>
              <button onClick={() => setTab("expedicao")} style={{ padding: "10px 20px", border: "none", borderRadius: "12px", cursor: "pointer", background: tab === "expedicao" ? "#fff" : "transparent", fontWeight: "800", color: tab === "expedicao" ? "#111" : "#888", boxShadow: tab === "expedicao" ? "0 4px 12px rgba(0,0,0,0.05)" : "none" }}>🚚 Expedição ({pedidos.filter(p => ["Pronto", "Saiu para Entrega"].includes(normalizarStatus(p.status))).length})</button>
              <button onClick={() => setTab("concluidos")} style={{ padding: "10px 20px", border: "none", borderRadius: "12px", cursor: "pointer", background: tab === "concluidos" ? "#fff" : "transparent", fontWeight: "800", color: tab === "concluidos" ? "#111" : "#888", boxShadow: tab === "concluidos" ? "0 4px 12px rgba(0,0,0,0.05)" : "none" }}>✅ Concluídos</button>
            </div>
        ...
        <div style={{ textAlign: "right", background: "#fff", padding: "15px", borderRadius: "15px", border: "1px solid #eee" }}>
            ...
            <span style={{ fontSize: "11px", color: "#4caf50", fontWeight: "bold" }}>● Gestor 4.1.2 Online 🔊</span>
        </div>
      </header>
      {loading ? (
          ...
          {pedidosFiltrados.map(pedido => (
            <div key={pedido.id} style={{ background: "#fff", border: "1px solid #eee", borderRadius: "25px", padding: "25px", boxShadow: "0 15px 35px rgba(0,0,0,0.03)", display: "flex", flexDirection: "column", position: "relative", overflow: "hidden" }}>
              ...
              <div style={{ marginBottom: "20px" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                    <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
                      ...
                      {/* --- DESTAQUE DO TELEFONE AMARELO (RESTAURADO) --- */}
              ...
              <div style={{ display: "flex", gap: "12px" }}>
                {tab === "cozinha" && (normalizarStatus(pedido.status) === "Pendente") && (
                  <button onClick={() => updateStatus(pedido.id, "Em Produção")} style={{ flex: 1, padding: "16px", borderRadius: "16px", border: "2px solid #111", background: "#fff", color: "#111", fontWeight: "900", cursor: "pointer", fontSize: "14px", transition: "0.2s" }}>ACEITAR PEDIDO</button>
                )}
                {tab === "cozinha" && normalizarStatus(pedido.status) === "Em Produção" && (
                  <button onClick={() => updateStatus(pedido.id, "Pronto", pedido)} style={{ flex: 1, padding: "16px", borderRadius: "16px", border: "none", background: "#4caf50", color: "#fff", fontWeight: "900", cursor: "pointer", fontSize: "14px", boxShadow: "0 6px 15px rgba(76, 175, 80, 0.3)" }}>MARCAR COMO PRONTO</button>
                )}
                {tab === "expedicao" && normalizarStatus(pedido.status) === "Pronto" && (
                ...
                {tab === "expedicao" && normalizarStatus(pedido.status) === "Saiu para Entrega" && (
                  <button onClick={() => updateStatus(pedido.id, "Finalizado")} style={{ flex: 1, padding: "16px", borderRadius: "16px", border: "none", background: "#111", color: "#fff", fontWeight: "900", cursor: "pointer", fontSize: "14px" }}>CONCLUIR PEDIDO</button>
                )}
              </div>
            </div>
          ))}
