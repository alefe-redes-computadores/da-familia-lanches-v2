...
export default function AdminPage() {
  ...
  // 🔊 FUNÇÃO PARA TOCAR ALERTA PERSISTENTE (ESTILO IFOOD)
    // 🔊 FUNÇÃO PARA TOCAR ALERTA PERSISTENTE (ESTILO IFOOD) - ATUALIZADA
  const controlarAlarme = (ligar: boolean) => {
    if (ligar) {
      ...
      // Verifica a cada 4 segundos se AINDA existem pedidos pendentes para repetir o som
      setTimeout(() => {
        setPedidos(atual => {
  ...
  // 🖨️ FUNÇÃO DE IMPRESSÃO PROFISSIONAL (HTML Completo e Estilizado)
  ...
  // 1. ESCUTAR PEDIDOS EM TEMPO REAL + LÓGICA DE SOM PERSISTENTE
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      // Conta apenas pedidos com status normalizado como "Pendente"
      ...
      if (!isFirstLoad.current && novosPedidosCount > prevPedidosCount.current) {
        controlarAlarme(true); // Ativa o loop de som
      }

      // Se não houver mais pendentes, silencia o alarme
    ...
    <div style={{ padding: "20px", maxWidth: "1200px", margin: "0 auto", fontFamily: "sans-serif", backgroundColor: "#fcfcfc", minHeight: "100vh" }}>
      ...
      <header style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "30px", borderBottom: "1px solid #eee", paddingBottom: "20px" }}>
        ...
        <div style={{ textAlign: "right" }}>
            ...
            <span style={{ fontSize: "11px", color: "#666" }}>Gestor 4.1.2 Ativo 🔊</span>
        </div>
      </header>
            ...
            <div key={pedido.id} style={{ 
              ...
              <div style={{ marginBottom: "15px" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                    <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
                      ...
                      {/* --- DESTAQUE TELEFONE AMARELO --- */}
                                            {/* --- DESTAQUE TELEFONE AMARELO (RESTAURADO COM BUSCA AMPLA) --- */}
                      <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                        {(pedido.userPhone || pedido.phone || pedido.telefone || pedido.contato) ? (
                        ...
                        <button 
                          onClick={() => {
                            let numRaw = pedido.userPhone || pedido.phone || pedido.telefone || pedido.contato;
                ...
                {(pedido.itens || []).map((item: any, idx: number) => (
                  <div key={idx} style={{ marginBottom: "10px", borderBottom: idx !== pedido.itens.length - 1 ? "1px solid #eee" : "none", paddingBottom: "8px" }}>
                    <div style={{ display: "flex", justifyContent: "space-between" }}>
                        ...
                        <span style={{ fontSize: "13px", color: "#888" }}>R$ {(Number(item.price || 0) * item.quantity).toFixed(2)}</span>
                    </div>
              ...
              <div style={{ display: "flex", gap: "10px" }}>
                ...
                {tab === "cozinha" && normalizarStatus(pedido.status) === "Em Produção" && (
                  <button onClick={() => updateStatus(pedido.id, "Pronto", pedido)} style={{ flex: 1, padding: "14px", borderRadius: "14px", border: "none", background: "#4caf50", color: "#fff", fontWeight: "900", cursor: "pointer", fontSize: "13px" }}>PRONTO</button>
                )}
                {tab === "expedicao" && (
