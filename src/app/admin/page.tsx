    ...
    <div style={{ padding: "20px", maxWidth: "1200px", margin: "0 auto", fontFamily: "sans-serif", backgroundColor: "#fcfcfc", minHeight: "100vh" }}>
      ...
      <header style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "30px", borderBottom: "1px solid #eee", paddingBottom: "20px" }}>
        <div>
          ...
          <div style={{ display: "flex", gap: "15px", marginTop: "15px", alignItems: "center", flexWrap: "wrap" }}>
            ...
            <div style={{ display: "flex", background: "#eee", padding: "4px", borderRadius: "12px" }}>
              ...
              >
                🚚 Expedição ({pedidos.filter(p => ["Pronto", "Saiu para Entrega"].includes(normalizarStatus(p.status))).length})
              </button>
              <button 
        ...
        <div style={{ textAlign: "right" }}>
            ...
            <span style={{ fontSize: "11px", color: "#666" }}>Gestor 4.2.0 CORRIGIDO 🔊</span>
        </div>
      </header>
              ...
              <div key={pedido.id} style={{ 
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "15px" }}>
                  <span style={{ 
                      ...
                      {normalizarStatus(pedido.status).toUpperCase()}
                  </span>
                ...
                <div style={{ marginBottom: "15px" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                      <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
                        <h3 style={{ margin: "0", fontSize: "20px", fontWeight: "700" }}>{pedido.userName || "Cliente"}</h3>
                        
                        <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                          {(pedido.userPhone || pedido.phone || pedido.telefone) ? (
                            <span style={{ background: "#ffca28", padding: "4px 10px", borderRadius: "8px", fontWeight: "bold", fontSize: "14px", color: "#000" }}>
                              📞 {pedido.userPhone || pedido.phone || pedido.telefone}
                            </span>
                          ) : (
                            <span style={{ fontSize: "12px", color: "#f44336", fontWeight: "bold" }}>Telefone não informado</span>
                          )}
                          
                          <button 
                            onClick={() => {
                              let numRaw = pedido.userPhone || pedido.phone || pedido.telefone;
                              let f = numRaw?.replace(/\D/g, "");
                  ...
                  <div style={{ marginTop: "10px", padding: "10px", borderRadius: "12px", background: pedido.tipoEntrega === "pickup" ? "#e3f2fd" : "#fff3e0", color: "#444", fontSize: "13px", border: "1px solid", borderColor: pedido.tipoEntrega === "pickup" ? "#bbdefb" : "#ffe0b2" }}>
                      <b>📍 {pedido.tipoEntrega === "pickup" ? "🥡 RETIRADA" : "🛵 ENTREGA"}</b><br/>{pedido.endereco || "Endereço não informado"}
                  </div>
                </div>

                <div style={{ background: "#f8f9fa", borderRadius: "15px", padding: "15px", marginBottom: "15px", flex: 1 }}>{itensValidos.length === 0 ? (
                    <div style={{ textAlign: "center", color: "#999", fontSize: "13px" }}>⚠️ Nenhum item no pedido</div>
                  ) : (
                    itensValidos.map((item: any, idx: number) => (
                      <div key={idx} style={{ marginBottom: "10px", borderBottom: idx !== itensValidos.length - 1 ? "1px solid #eee" : "none", paddingBottom: "8px" }}>
                        <div style={{ display: "flex", justifyContent: "space-between" }}>
                            <span style={{ fontSize: "14px", fontWeight: "600" }}>{item.quantity || 1}x {item.name || "Item"}</span>
                ...
                <div style={{ marginBottom: "20px", borderTop: "2px dashed #eee", paddingTop: "15px" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", color: "#666", fontSize: "13px", marginBottom: "5px" }}>
                      <span>{pedido.metodoPagamento?.toUpperCase() || "N/A"}</span>
                ...
                <div style={{ display: "flex", gap: "10px" }}>
                  ...
                  {tab === "cozinha" && normalizarStatus(pedido.status) === "Em Produção" && (
                    <button onClick={() => updateStatus(pedido.id, "Pronto", pedido)} style={{ flex: 1, padding: "14px", borderRadius: "14px", border: "none", background: "#4caf50", color: "#fff", fontWeight: "900", cursor: "pointer", fontSize: "13px" }}>
                      {pedido.tipoEntrega === "pickup" ? "PRONTO (AVISAR ZAP)" : "PRONTO P/ EXPEDIÇÃO"}
                    </button>
                  )}
