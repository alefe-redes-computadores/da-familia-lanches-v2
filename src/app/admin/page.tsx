...
export default function AdminPage() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      console.log("📊 Pedidos carregados:", docs.length);
      console.log("🔔 Pedidos pendentes:", novosPedidosCount);
  ...
  // 🔧 CORREÇÃO PRINCIPAL: Lógica de filtragem SIMPLIFICADA (igual ao código antigo)
  const pedidosFiltrados = pedidos.filter(p => {
    const s = p.status || "Pendente";
    
    if (tab === "cozinha") {
      return s === "Pendente" || s === "Em Produção";
    }
  ...
  console.log(`📋 Aba "${tab}": ${pedidosFiltrados.length} pedidos`);
    ...
    <div style={{ padding: "20px", maxWidth: "1200px", margin: "0 auto", fontFamily: "sans-serif", backgroundColor: "#fcfcfc", minHeight: "100vh" }}>
      ...
      <header style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "30px", borderBottom: "1px solid #eee", paddingBottom: "20px" }}>
        <div>
          ...
          <div style={{ display: "flex", gap: "15px", marginTop: "15px", alignItems: "center", flexWrap: "wrap" }}>
            ...
            <div style={{ display: "flex", background: "#eee", padding: "4px", borderRadius: "12px" }}>
              ...
              <button onClick={() => setTab("concluidos")} style={{ padding: "8px 16px", border: "none", borderRadius: "10px", cursor: "pointer", background: tab === "concluidos" ? "#fff" : "transparent", fontWeight: "bold", color: tab === "concluidos" ? "#111" : "#666" }}>✅ Finalizados ({pedidos.filter(p => p.status === "Finalizado").length})</button>
            </div>
        ...
        <div style={{ textAlign: "right" }}>
          ...
          <span style={{ fontSize: "11px", color: "#666" }}>Gestor 4.1.1 Ativo 🔊</span>
        </div>
      </header>
      ...
      {loading ? (
        ...
        <div style={{ textAlign: "center", padding: "50px", color: "#999" }}>
          <p style={{ fontSize: "18px" }}>📭 Nenhum pedido nesta aba</p>
        </div>
      ) : (
