...
const eventTypeSet = new Set<string>(INTEGRATION_EVENT_TYPES);
const entityTypeSet = new Set<string>(INTEGRATION_ENTITY_TYPES);
...
export function buildIntegrationEvent<TPayload>(
  ...
  if (!eventTypeSet.has(input.event_type)) {
    throw new Error(
      `event_type inválido: ${input.event_type}.`,
    );
  }

  if (!entityTypeSet.has(input.entity_type)) {
    throw new Error(
      `entity_type inválido: ${input.entity_type}.`,
    );
  }

  const sourceSystem =
    input.source_system || "dfl_site";

  if (!sourceSystems.has(sourceSystem)) {
    throw new Error(
      `source_system inválido: ${sourceSystem}.`,
    );
  }
...
export function isIntegrationEventEnvelope(
  ...
  return (
    ...
    candidate.schema_version === INTEGRATION_SCHEMA_VERSION &&
