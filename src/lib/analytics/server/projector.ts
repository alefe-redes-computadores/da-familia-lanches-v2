...
type Contribution={dateKey:string;sales:number;revenue:number;subtotal:number;deliveryFees:number;discounts:number;delivery:number;pickup:number;scheduled:number;immediate:number;logisticsCompleted:number;payments:NumMap;products:NumMap};
const empty=(dateKey:string):Contribution=>({dateKey,sales:0,revenue:0,subtotal:0,deliveryFees:0,discounts:0,delivery:0,pickup:0,scheduled:0,immediate:0,logisticsCompleted:0,payments:{},products:{}});
...
function contribution(o:Record<string,unknown>):Contribution{
 ...
 c.payments[key(o.metodoPagamento)]=c.revenue;
...
export async function projectOrderById(orderId:string,sourceEventId?:string){
 const orderRef=adminDb.collection("Pedidos").doc(orderId),stateRef=adminDb.collection(STATE).doc(encodeURIComponent(orderId));
 return adminDb.runTransaction(async tx=>{
  const [os,ss]=await Promise.all([tx.get(orderRef),tx.get(stateRef)]);if(!os.exists)return{projected:false,reason:"order_not_found"};
  const next=contribution(os.data()||{}),prev=ss.exists?(ss.data()?.contribution as Contribution|undefined):undefined;
  if(prev&&same(prev,next)){if(sourceEventId)tx.set(stateRef,{lastSourceEventId:sourceEventId,updatedAt:new Date().toISOString()},{merge:true});return{projected:false,reason:"unchanged",dateKey:next.dateKey}}
  ...
  tx.set(stateRef,{orderId,authority:"dfl_site",businessKey:`dfl_site:order:${orderId}`,contribution:next,projectorVersion:PROJECTOR_VERSION,lastSourceEventId:sourceEventId||null,updatedAt:now},{merge:false});
...
export async function runAnalyticsBackfill(batchSize=80){
 ...
 const ref=adminDb.collection(CHECKPOINTS).doc("site_orders_v2"),snap=await ref.get(),d=snap.data()||{};if(d.complete===true){backfillCompleteInProcess=true;return{complete:true,processed:0,cached:false};}
 ...
 const page=await q.get();let processed=0,last=cursor;for(const doc of page.docs){await projectOrderById(doc.id,"backfill-v2");processed++;last=doc.id}
 const complete=page.size<size;await ref.set({projectorVersion:PROJECTOR_VERSION,lastDocumentId:last||null,processedTotal:num(d.processedTotal)+processed,complete,updatedAt:new Date().toISOString(),completedAt:complete?new Date().toISOString():null},{merge:true});
 if(complete)backfillCompleteInProcess=true;
