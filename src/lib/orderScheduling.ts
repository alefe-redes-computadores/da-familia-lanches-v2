...
export async function getOrderScheduleSlots(daysAhead=7):Promise<OrderScheduleSlot[]>{
 ...
 const dates=[...new Set(list.map(x=>x.date))],used=new Map<string,number>();for(const date of dates){const q=await getDocs(query(collection(db,"schedule_slots"),where("date","==",date)));q.forEach(x=>used.set(x.id,Math.max(0,Number(x.data().reserved)||0)))}
