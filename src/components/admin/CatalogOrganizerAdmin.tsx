...
export function CatalogOrganizerAdmin(){
 ...
 const visibleCategories=categories.filter(c=>c.active);const active=(category&&visibleCategories.some(c=>c.id===category)?category:"")||visibleCategories[0]?.id||"";
 const rows=useMemo(()=>[...products.filter(p=>p.category===active&&p.disponivel!==false)].sort((a,b)=>mode==="price-asc"?a.price-b.price||a.name.localeCompare(b.name,"pt-BR"):mode==="price-desc"?b.price-a.price||a.name.localeCompare(b.name,"pt-BR"):mode==="alpha"?a.name.localeCompare(b.name,"pt-BR"):(a.sortOrder??Number.MAX_SAFE_INTEGER)-(b.sortOrder??Number.MAX_SAFE_INTEGER)||a.name.localeCompare(b.name,"pt-BR")),[products,active,mode]);
 ...
 return <section className={styles.root}><header><span>ORGANIZAÇÃO DO CARDÁPIO</span><h3>Sequência por categoria</h3><p>Ordene os produtos disponíveis dentro de cada categoria visível. Itens pausados ficam fora da sequência pública.</p></header>
 <div className={styles.controls}><label>Categoria<select value={active} onChange={e=>{setCategory(e.target.value);setMode("manual")}}>{visibleCategories.map(c=><option key={c.id} value={c.id}>{c.label}</option>)}</select></label>
 ...
 <div className={styles.list}>{rows.map((p,i)=><article key={p.id} data-paused={!p.disponivel}><b>{i+1}</b><img src={p.image} alt=""/><div><strong>{p.name}</strong><span>{p.price.toLocaleString("pt-BR",{style:"currency",currency:"BRL"})}</span></div><nav><button disabled={busy||i===0} onClick={()=>move(p.id,-1)}>↑</button><button disabled={busy||i===rows.length-1} onClick={()=>move(p.id,1)}>↓</button></nav></article>)}</div>{msg&&<div className={styles.msg}>{msg}</div>}</section>
}
