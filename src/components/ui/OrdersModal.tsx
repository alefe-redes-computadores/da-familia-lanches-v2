...
export function OrdersModal(){
 ...
 const {orders,activeOrders,pastOrders,loading,error}=useCustomerOrders(currentUser); const [notice,setNotice]=useState(""); const [repeatTarget,setRepeatTarget]=useState<string|null>(null); const [tab,setTab]=useState<"active"|"history">("active");
 ...
 return <ModalBase title="Meus pedidos" onClose={closeModal}><div className={styles.body}>
   ...
   <div className={styles.list}>{visible.map(order=>{const items=getOrderItems(order),pickup=order.tipoEntrega==="pickup",status=normalizarStatus(order.status),steps=timelineSteps(pickup),active=!TERMINAL.has(status),updated=lastStatusTime(order);return <article className={`${styles.card} ${active?styles.activeCard:""}`} key={order.id}>
    ...
    {status==="Agendado"&&<div className={styles.scheduled}>O pedido está registrado para atendimento posterior. Quando a equipe iniciar, o progresso aparece aqui.</div>}
